<template>
    <v-container class="main">
        <v-row>
                ...
                <v-row>
                    <v-col>
                        <div class="assemblyTable">
                            <FormContainer ref="formContainer" @data-emitted-from-b="updateComponent" />
                        </div>
                    </v-col>
                </v-row>
            </v-col>
            <v-col cols="2">
                <div class="attribute">
                    <FormDetail :receivedData="receivedData" />
                </div>
            </v-col>
        </v-row>
    </v-container>
...
export default {
    ...
    mounted() {
        // 在页面实例的 mounted 钩子中使用 $on 监听来自组件 B 的事件
        this.$root.$on("data-emitted-from-b", this.updateComponent);
    },
}
</script>
